...
my $entry = $ARGV[1]; #article;
my $headword =$ARGV[2];#mot_vedette;
my $pos = $ARGV[3]; #catégorie_grammaticale;

my $lines = 0;
$/ = "<\/$entry>";
open my $FILE, "<:encoding($encoding)","$ARGV[0]" or die ("$! IN \n");
open my $OUT, ">:encoding($encoding)","$ARGV[4]" or die ("$! OUT \n");
my @lines = <$FILE>;
my @dico = sort { 
