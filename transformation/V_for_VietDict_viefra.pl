...
# Remarques : - Les entrées sont en vietnamien (traduction en français)
#             - Création d'un journal des exécutions (LOG)
# ---------------------------------------------------------------------------------------------------------------------------------------
# Usage : perl V_VietDict_viefra.pl -v -from source.xml -to MAM.xml 
#
...
my $ref_sense = 'semantic-sense' ; # idem pour l'élément <m:sense> dans MAM.

my  $in_root 		   = 'volume'; 
my  $in_entry 		   = 'entry'; 		
my	$in_headword 	   = 'headword';  	
my	$in_sense 		   = 'semantic-sense'; 	   
...
my ($verbeux, $help) = ();
my $count_entry = 8686000000; # 8670 = VV (Pour Vietdict Vietnamien).
GetOptions( 
...
$writer->startTag(
	"m:entry", 
	'id' => 'vie.' . $twig_entry->field($in_headword) . '.' . $count_entry, 
	'level' => ""
	);
