...
if ($verbeux) {print STDERR "Extrait les vedettes avec cdmheadword: $cdmheadword, et cat avec cdmpos: $cdmpos\n";}

my $nbentries = 0;

my @total = ();
my $twig = 'XML::Twig'->new(twig_handlers => { $cdmentry => \&push_entries });
if ($FichierEntree) {
...
my @dico = sort { 
	$collator->cmp(${ $a }[0],${ $b }[0]) or $collator->cmp(${ $a }[1],${ $b }[1]);	
	} 
	@total;
...
sub push_entries {
	my( $twig, $entry)= @_;           
	my $headword = $entry->findvalue($cdmheadword);
	my $pos = $entry->findvalue($cdmpos);
	$entry = $entry->sprint();
	$twig->purge();
	$entry =~ s/\R/ /gsm;
	my @array = ($headword, $pos, $entry);
	push @total, \@array;
